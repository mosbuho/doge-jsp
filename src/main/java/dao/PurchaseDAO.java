...
public class PurchaseDAO {
	...
	public void purchase(int member_id, int goods_id, int quantity, String name, String addr, String uuid) {
		try (Connection conn = DBUtil.getConnection();
				PreparedStatement pstmt = conn.prepareStatement(
						"insert into purchase (member_id, goods_id, quantity, name, addr, transaction_id) values (?, ?, ?, ?, ?, ?)")) {
	...
	public HashMap<String, ArrayList<PurchaseBean>> getPurchaseList(int member_id) {
		...
		try (Connection conn = DBUtil.getConnection();
				PreparedStatement pstmt = conn.prepareStatement(
						"select p.purchase_id, p.member_id, p.goods_id, p.quantity, p.name, p.addr, p.delivery_state, p.transaction_id, p.reg_date, g.title_img, g.title, g.price "
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					PurchaseBean purchase = new PurchaseBean(rs.getInt("purchase_id"), rs.getInt("member_id"),
							...
							rs.getInt("delivery_state"), transaction_id, rs.getDate("reg_date"),
							rs.getString("title_img"), rs.getString("title"), rs.getInt("price"));
